...
--- Fetching real script data from the API ---")
        ...
        for category in data:
            ...
            for script in category_scripts:
                ...
                # Simple version of _slugify for analysis
                import re, unicodedata
                value = unicodedata.normalize("NFKD", slug)
        ...
        serialized_scripts = json.dumps(scripts, ensure_ascii=False).encode("utf-8")
        pre_compressed_size = len(serialized_scripts)
        print(f"Pre-compressed (JSON) size: {pre_compressed_size} bytes")

        compressed_scripts = zlib.compress(serialized_scripts, level=6)
...
# for direct script invocation (e.g., `python tools/analyze_api.py`).
